...
def get_page(url, page, collection_handle=None):
    ...
    products = json.loads(data.decode('utf-8'))['products']
    return products
def get_page_collections(url):
    ...
    while True:
        ...
        cols = json.loads(data.decode('utf-8'))['collections']
        if not cols:
            break
        for col in cols:
            yield col
...
def extract_products_collection(url, col):
    ...
    while products:
        for product in products:
            ...
            # Joining the image URLs into a single string separated by ';'
            all_images = ';'.join(image_urls)

            # Get the product description and remove any HTML tags (like <p>, <br>, etc.)
            description_raw = product.get('body_html', '')
    ...
    if len(args) > 0:
        ...
        if options.list_collections:
            ...
            with open('products.csv', 'w') as f:
                writer = csv.writer(f)
                writer.writerow(['Code', 'Collection', 'Category', 'Name', 'Variant Name', 'Price', 
                                 'In Stock', 'URL', 'Image URL', 'Description', 'Body'])
                seen_variants = set()
                for col in get_page_collections(url):
                    ...
                    for product in extract_products_collection(url, handle):
                        ...
                        writer.writerow([
                            ...
                            product['image_src'], product['description'],
                            product['body']
                        ])
